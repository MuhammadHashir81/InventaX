...
const buildInvoiceMatch = ({ startDate, endDate, customerId, tenantId }) => {
    const match = {};
    if (tenantId) {
...
const buildOutflowMatch = ({ startDate, endDate, tenantId }) => {
    const match = {};
...
const getDashboardContext = async (payload = {}, userId) => {
    ...
    return {
        ...
        customerId,
        tenantId,
        invoiceMatch: buildInvoiceMatch({ startDate, endDate, customerId, tenantId }),
        outflowMatch: buildOutflowMatch({ startDate, endDate, tenantId }),
    };
};
...
const handleGetDashboardSummary = async (req, res) => {
    try {
        const context = await getDashboardContext(req.body, req.userId);
...
const handleGetTopProducts = async (req, res) => {
    ...
    try {
        ...
        if (context.productId) {
            pipeline.push({
                $match: { "product.productId": context.productId },
            });
        }
