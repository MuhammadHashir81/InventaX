...
const Dashboard = () => {
  ...
  const fetchDashboardData = async (overrides = {}) => {
    try {
      ...
      console.log('top products...',topProductsResponse, 'top customers...',topCustomersResponse )
              ...
              {topProducts.length ? (
                topProducts.map((product) => (
                  <div key={product.productId || product.name} className='flex items-center justify-between'>
                    ...
                    <p className='font-primary font-medium text-green-500 my-1'>{product.totalSales} PKR</p>
                  </div>
                ))
              ) : (
              ...
              {topCustomers.length ? (
                topCustomers.map((customer) => (
                  <div key={customer.customerId || customer.name} className='flex items-center justify-between'>
                    ...
                    <p className='font-primary font-medium text-green-500 my-1'>{customer.totalSales} PKR </p>
                  </div>
                ))
              ) : (
